...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="2.0" xmlns:dp="http://www.datapower.com/extensions" xmlns:regexp="http://exslt.org/regular-expressions" xmlns:date="http://exslt.org/dates-and-times" xmlns:soap="http://schemas.xmlsoap.org/soap/envelope/" extension-element-prefixes="dp date regexp" exclude-result-prefixes="dp  date soap" xmlns:xsd="http://sml.com.vn/xsd" xmlns:dpconfig="http://www.datapower.com/param/config">
	...
	<!--
	<xsl:param name="dpconfig:SignWS" select="'NULL'"/>
	<xsl:param name="dpconfig:NapasWS" select="'NULL'"/>
	-->
	<xsl:template match="/">
		<!--SIMUALATOR-->		
		<!--UAT
		...
		<xsl:variable name="ProcessingCode">050000</xsl:variable>
		...
		<xsl:variable name="input"><xsl:value-of select="$MTI" />|<xsl:value-of select="$PrimaryAccountNumber" />|<xsl:value-of select="$ProcessingCode" />|<xsl:value-of select="$TransactionAmount" />|<xsl:value-of select="$TransmissionDateTime" />|<xsl:value-of select="$AuditNumber" />|<xsl:value-of select="$MerchantType" />|<xsl:value-of select="$AcquiringCode" />|<xsl:value-of select="$TermId" />|<xsl:value-of select="$CardAcceptorName" />|<xsl:value-of select="$ServiceCode" />|<xsl:value-of select="$BillId" /></xsl:variable>
		...
		<xsl:variable name="term_id" select="substring-before( $response10, '|' )" />
		...
		<xsl:variable name="Bill_id" select="substring-before( $response12, '|' )" />
		<xsl:variable name="response13" select="substring-after( $response12, '|' )" />
		<xsl:variable name="AdditionalData" select="substring-before( $response13, '|' )" />
		...
		<soapenv:Envelope xmlns:soapenv="http://schemas.xmlsoap.org/soap/envelope/" xmlns:sml="http://www.scb.com.vn/billpayment/sml">
		   ...
		   <soapenv:Body>
			  <sml:getBillInfoResponse>
				 ...
				 <TransactionAmount><xsl:value-of select="number(substring($Transaction_Amount, 1, string-length($Transaction_Amount) - 2)) * 1" /></TransactionAmount>
				 <AuthorizationCode><xsl:value-of select="$Authorization_Code" /></AuthorizationCode>
				 <AdditionalData><xsl:value-of select="$AdditionalData" /></AdditionalData>				 
				 <Payload>
